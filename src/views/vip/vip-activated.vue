<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="导入日期" class-name="status-col" width="160" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.ImportTimeStr }}</span>
        </template>
      </el-table-column>
        <el-table-column label="激活时间" class-name="status-col" width="160" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.LastActiveCodeTimeStr }}</span>
        </template>
      </el-table-column>
       <el-table-column label="激活人" width="110px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.LastActiveCodeUserID }}</span>
        </template>
      </el-table-column>
      <el-table-column label="是否绑定" width="100px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.IsBind == 1?'是':'否' }}</span>
        </template>
      </el-table-column>
       <el-table-column label="是否放弃" width="100px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.IsGiveUp == 1?'是':'否' }}</span>
        </template>
      </el-table-column>
       <el-table-column label="是否完成训练" width="110px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.IsPanlComplete == 1?'是':'否' }}</span>
        </template>
      </el-table-column>
       <el-table-column label="首访日期" class-name="status-col" width="160" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.FirstVisitTimeStr }}</span>
        </template>
      </el-table-column>
      <el-table-column label="已训练次数" width="100px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.PlanComplete }}</span>
        </template>
      </el-table-column>
      <el-table-column label="末次训练时间" class-name="status-col" width="160" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" width="130" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          ...
          <el-button v-if="row.status!='deleted'" size="mini" type="primary" @click="handleUpdate(row)">
            查看详情
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="110px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="激活时间">
          <el-input v-model="temp.LastActiveCodeTimeStr" placeholder="" readonly/>
        </el-form-item>
        <el-form-item label="激活人">
          <el-input v-model="temp.LastActiveCodeUserID" placeholder="" readonly/>
        </el-form-item>
        <el-form-item label="是否绑定">
          <span>{{ temp.IsBind }}</span>
        </el-form-item>
